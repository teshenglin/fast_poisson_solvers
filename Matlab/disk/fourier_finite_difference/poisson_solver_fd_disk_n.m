...
%    hh: Neumann boundary condition at r=1, function handle
...
%%
function u = poisson_solver_fd_disk_n(M, N, hh, f)
    ...
    % special treatment for zero Fourier mode
    ...
    % for non-zero Fourier modes
    for ii = 2:N/2+1
        
        % setup of RHS
        RHS = dr^2*(r.*fft_f(ii,:)).';
        RHS(M) = RHS(M)-dr*fft_h(ii);
